...
const (
	...
	kubeletCAAnnotation = "some-name.olm.openshift.io/kubelet-ca-configmap-hash"
	dataVolumeName      = "profiledata"
	dataMountPath       = "/run/some-mountpath"
)
...
func (r *SomeReconciler) ensureDaemonSet(ctx context.Context, someName *v1alpha2.SomeAPI, sa *corev1.ServiceAccount, ns string, kubeletCAConfigMap *corev1.ConfigMap) (*appsv1.DaemonSet, error) {
	kubeletCAHash, err := buildMapHash(kubeletCAConfigMap.Data)
	if err != nil {
		return nil, fmt.Errorf("failed to build hash of kubelet ca: %w", err)
	}
	desired := r.desiredDaemonSet(someName, sa, ns, kubeletCAConfigMap.Name, kubeletCAHash)
...
func (r *SomeReconciler) currentDaemonSet(ctx context.Context, nameSpace types.NamespacedName) (*appsv1.DaemonSet, error) {
	ds := &appsv1.DaemonSet{}
...
func (r *SomeReconciler) createDaemonSet(ctx context.Context, ds *appsv1.DaemonSet) error {
	return r.Create(ctx, ds)
}

func (r *SomeReconciler) updateDaemonset(ctx context.Context, current, desired *appsv1.DaemonSet) (bool, error) {
...
func (r *SomeReconciler) desiredDaemonSet(someName *v1alpha2.SomeAPI, sa *corev1.ServiceAccount, ns, kubeletCAConfigMapName, kubeletCAHash string) *appsv1.DaemonSet {
	ls := labelsForSomeName(someName.Name)
		...
		Spec: appsv1.DaemonSetSpec{
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					NodeSelector: someName.Spec.NodeSelector,
				},
			},
		},
	}
...
func (r *SomeReconciler) purgeObsoleteDaemonset(ctx context.Context, name types.NamespacedName) error {
	ds := &appsv1.DaemonSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name.Name,
